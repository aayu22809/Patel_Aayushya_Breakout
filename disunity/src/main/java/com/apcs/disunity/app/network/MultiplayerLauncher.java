...
public class MultiplayerLauncher implements Closeable {
    ...
    public MultiplayerLauncher(Consumer<Boolean> initNodes) { this.initNodes = initNodes; }

    public boolean lauch() throws IOException {
        String input = JOptionPane
                .showInputDialog("DISUNITY STARTUP WIZARD\n\nWould you like to start or join a session? (s/j)");
        if (input == null)
            System.exit(0);
    ...
    public void launchServer() throws IOException {
        ...
        boolean forward = JOptionPane.showInputDialog(
                "DISUNITY STARTUP WIZARD\n\nWould you like to expose to the internet in addition to LAN?\n Doing so requires bash and ssh, but allows players on different networks to join. (y/n)")
                .startsWith("y");
        if (forward) {
        ...
        JOptionPane.showMessageDialog(null,
                String.format("JOIN INFORMATION:\n\nAddress: %s\nPort: %s\n\nClose this window to end the instance",
                        lastLine.substring(Math.max(lastLine.lastIndexOf(' '), 0)).split(":")));
        if (process != null)
            process.destroy();
        host.close();
        System.exit(0);
